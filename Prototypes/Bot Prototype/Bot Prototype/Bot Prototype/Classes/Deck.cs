...
namespace Deck_Prototype.Classes
{
    ...
    public class Deck
    {
        ...
        public void FillStandardUnus()
        {
            ...
            for (int j = 0; j < 4; j++)
            {
                for (int i = 0; i < 2; i++)
                {
                    for (int k = 1; k < 10; k++)
                    {
                        DrawPile.Add(new Card(k, (Enums.CardColor)j, Enums.CardType.Normal));
                    }
                }
            }
        ...
        /// <summary>
        /// Checks a player's hand for valid cards to play
        /// </summary>
        public List<int> CheckHand(Hand plr)
        {
            List<int> Indexes = new List<int>();
            int count = 0;
            Card Top = DiscardPile.Last();
            foreach(Card card in plr.Cards)
            {
                if (CheckCard(card, Top))
                {
                    Indexes.Add(count);
                }
                count++;
        ...
        public bool CheckCard(Card card, Card Top)
        {
            if (card.Type == Enums.CardType.Color || card.Type == Enums.CardType.ColorPlusFour)
            {
                return true;
            }
